...
public class Assignment2{
    ...
    static void loseWeight(int currentWeight) {
        ...
        if (currentWeight > 150) { // Define the base case by using an if condition. The target weight is 150 lbs
            currentWeight = fixDietandExercise(currentWeight); // Do something to reach the target. Call the fixDietandExercise() method
            loseWeight(currentWeight); // Do the recursive call. Call the loseWeight() method
        }  
      
        else if(currentWeight == 150){// if the target weight has been reached, print the code below.
            System.out.println("I have reached my target weight!"); // System.out.println("I have reached my target weight!");
